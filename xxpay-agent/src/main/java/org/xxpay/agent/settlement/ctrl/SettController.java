...
public class SettController extends BaseController {
    ...
    @ResponseBody
    public ResponseEntity<?> settDownload(HttpServletRequest request) {
        JSONObject param = getJsonParam(request);
        SettRecord settRecord = getObject(param, SettRecord.class);
        String createTimeStartStr = getString(param, "createTimeStart");
        Date createTimeStart = DateUtil.str2date(createTimeStartStr);
        String createTimeEndStr = getString(param, "createTimeEnd");
        Date createTimeEnd = DateUtil.str2date(createTimeEndStr);
        int count = rpcCommonService.rpcSettRecordService.count(settRecord, getQueryObj(param));
        if (count == 0) return ResponseEntity.ok(XxPayPageRes.buildSuccess());
        List<SettRecord> settRecordList = rpcCommonService.rpcSettRecordService.selectByTimeRang((getPageIndex(param) - 1) * getPageSize(param), getPageSize(param), createTimeStart, createTimeEnd);
//        TODO 将结算记录导出到excel表格
        return ResponseEntity.ok(XxPayPageRes.buildSuccess(settRecordList, count));              // 费用
    }
