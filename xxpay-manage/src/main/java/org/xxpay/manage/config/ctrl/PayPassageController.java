...
//            String fixAmountNew = Arrays.stream(fixAmount.split(",")).map(e -> Long.parseLong(e)).map(e -> e * 100).map(e -> e.toString()).reduce((e, b) -> e + "," + b).get();
//            payPassage.setFixAmount(fixAmountNew);
//        }
        int count = rpcCommonService.rpcPayPassageService.update(payPassage);
        if (count == 1) return ResponseEntity.ok(BizResponse.buildSuccess());
        return ResponseEntity.ok(BizResponse.build(RetEnum.RET_COMM_OPERATION_FAIL));
