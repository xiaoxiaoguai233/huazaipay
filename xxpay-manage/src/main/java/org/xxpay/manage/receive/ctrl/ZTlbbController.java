...
public class ZTlbbController extends BaseController {
    ...
    public ResponseEntity<?> getlist(HttpServletRequest request) {
        ...
        int count = rpcCommonService.rpcZMthAccountService.selectListCount(zTlbbAccount.getParentId());

        for(ZTlbbAccount zTlbbAccount_ : zTlbbAccounts){
            if (zTlbbAccount_.getParentId() >= 20000000){
                ...
                SysUser sysUser = rpcCommonService.rpcSysService.findByUserId(zTlbbAccount_.getParentId());
                zTlbbAccount_.setParentName(sysUser.getNickName());
            }
