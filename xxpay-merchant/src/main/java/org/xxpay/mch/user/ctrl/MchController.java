...
public class MchController extends BaseController {
    ...
    public ResponseEntity<?> updateKey(HttpServletRequest request) {
        ...
        MchInfo mchInfo = userService.findByMchId(getUser().getId());
        if (!checkGoogleCode(mchInfo.getGoogleAuthSecretKey(), code))
    ...
    @ResponseBody
    @MethodLog(remark = "获取收银台配置")
    public ResponseEntity<?> getCashierConfig(HttpServletRequest request) {
        Long merchantId = getUser().getId();
        CashierConfig cashierConfig = rpcCommonService.rpcCashierConfigService.get(merchantId);
        return ResponseEntity.ok(XxPayResponse.buildSuccess(cashierConfig));
    }
     ...
     * @return
     */
    @RequestMapping("/update_cashier")
    @ResponseBody
    @MethodLog(remark = "更新收银台配置")
    public ResponseEntity<?> updateCashierConfig(HttpServletRequest request) {
        Long name = getUser().getId();
        Boolean exists = true;
        CashierConfig cashierConfig = rpcCommonService.rpcCashierConfigService.get(name);
        if (cashierConfig == null) {
            exists = false;
        }
        JSONObject param = getJsonParam(request);
        cashierConfig = new CashierConfig();
        cashierConfig.setCallbackUrl(param.getString("callbackUrl"));
        cashierConfig.setReturnUrl(param.getString("returnUrl"));
        cashierConfig.setName(param.getString("name"));
        cashierConfig.setLogo(param.getString("logo"));
        cashierConfig.setStatus(param.getByte("status"));
        cashierConfig.setMerchantid(getUser().getId());
        Integer integer = exists ? rpcCommonService.rpcCashierConfigService.update(cashierConfig) : rpcCommonService.rpcCashierConfigService.add(cashierConfig);
        return ResponseEntity.ok(XxPayResponse.buildSuccess());
