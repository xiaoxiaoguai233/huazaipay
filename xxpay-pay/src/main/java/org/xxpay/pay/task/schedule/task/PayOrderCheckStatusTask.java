...
public class PayOrderCheckStatusTask implements Runnable {
    ...
    public void run() {
        ...
        String url = "http://127.0.0.1:8002/get_order_status?amount=" + amount + "&token=" + token + "&spsn=" + orderSn;
        // 获取支付链接
        String body = "";
        try {
        ...
        String dianshu = json.getString("dianshu");

        String check_dianshuh = String.valueOf(Integer.valueOf(amount) * 20);

        // 订单状态支付成功
        if (status.equals("1") && dianshu.equals(check_dianshuh)){
            // 获取订单的详细信息
            PayOrder payOrder = rpcCommonService.rpcPayOrderService.findByPayOrderId(payOrderId);

            // 更新收款账号信息，并且判断账号是否收款成功
            ZTlbbAccount zTlbbAccount = rpcCommonService.rpcZTlbbAccountService.selectByToken(token);
            zTlbbAccount.setTodayMoney(zTlbbAccount.getTodayMoney() + payOrder.getAmount());
            zTlbbAccount.setTotalMoney(zTlbbAccount.getTotalMoney() + payOrder.getAmount());
            // 如果收款金额达到日限额或者账号限额，状态设置为限额
            if (zTlbbAccount.getTodayMoney() > zTlbbAccount.getLimitDayMoney() || zTlbbAccount.getTotalMoney() >= zTlbbAccount.getLimitMaxMoney()){
                zTlbbAccount.setState((byte) 3);
            }
