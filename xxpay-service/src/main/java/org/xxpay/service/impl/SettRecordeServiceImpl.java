...
import org.xxpay.core.common.vo.ManagerSettOrderVO;
...
public class SettRecordeServiceImpl implements ISettRecordService {
    ...
    @Override
    public List<SettRecord> selectByTimeRang(int offset, int limit, Date startTime,Date endTime) {
        SettRecordExample example = new SettRecordExample();
        example.setOrderByClause("createTime desc");
        example.setOffset(offset);
        example.setLimit(limit);
    ...
    @Override
    public List<ManagerSettOrderVO> downSettList(SettRecord settRecord, JSONObject queryObj) {
        SettRecordExample example = new SettRecordExample();
        example.setOrderByClause("createTime desc");
//        example.setOffset(offset);
//        example.setLimit(limit);
        SettRecordExample.Criteria criteria = example.createCriteria();
        setCriteria(criteria, settRecord, queryObj);
        return settRecordMapper.downSettListByExample(example);
    }
