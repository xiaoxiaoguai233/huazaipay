...
public class MchAccountServiceImpl implements IMchAccountService {
    ...
    @Override
    public List<MchAccount> getAll(int offset, int limit, MchAccount mchAccount) {
        PageHelper.offsetPage(offset, limit);
        if("".equals(mchAccount.getName())){
            mchAccount.setName(null);
        }
        if("".equals(mchAccount.getMchId())){
            mchAccount.setMchId(null);
        }
        MchAccountExample example = new MchAccountExample();
        MchAccountExample.Criteria criteria = example.createCriteria();
        setCriteria(criteria, mchAccount);
        return mchAccountMapper.selectByExample(example);
    }

    void setCriteria(MchAccountExample.Criteria criteria, MchAccount mchAccount) {
        if (mchAccount != null) {
            if (mchAccount.getMchId() != null) criteria.andMchIdEqualTo(mchAccount.getMchId());
            if (mchAccount.getName() != null) criteria.andNameEqualTo(mchAccount.getName());
        }
    ...
    @Override
    public Map stat() {
        return mchAccountMapper.stat();
    }

    @Override
    public int count() {
        return mchAccountMapper.countByExample(null);
    }
    ...
    @Override
    @Transactional(transactionManager = "transactionManager", rollbackFor = Exception.class)
    public void credit2Account(Long mchId, Byte bizType, Long amount, String bizItem) {
        credit2Account(mchId, bizType, amount, MySeq.getChangeAccount(), null, null, bizItem);
    }
    ...
    @Override
    @Transactional(transactionManager = "transactionManager", rollbackFor = Exception.class)
    public void credit2Account(Long mchId, Byte bizType, Long amount, String orderId, Long orderAmount, Long fee, String bizItem) {
        MchInfo mchInfo = findAvailableInfo(mchId);
        MchAccount mchAccount = findAvailableAccount(mchId);
        MchAccount updateMchAccount = new MchAccount();
