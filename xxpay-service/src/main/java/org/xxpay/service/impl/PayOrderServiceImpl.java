...
public class PayOrderServiceImpl implements IPayOrderService {
    ...
    @Autowired private PayPassageMapper payPassageMapper;
    @Autowired private IMchAccountService mchAccountService;

    @Autowired private IAgentInfoService agentInfoService;
    ...
    @Override
    public int updateCardPwdByOrderId(String payOrderId, String cardPwd) {
        return payOrderMapper.updateCardPwdByOrderId(payOrderId, cardPwd);
    }
    ...
    @Override
    public Map count4All(Long agentId, Long mchId, Long productId, String payOrderId, String mchOrderNo, Byte productType, String createTimeStart, String createTimeEnd,String channelId,String channelType,String subMchId, String phone, String cardPwd) {
        Map param = new HashMap<>();
        if(agentId != null) param.put("agentId", agentId);
        if(mchId != null) param.put("mchId", mchId);
        ...
        if(StringUtils.isNotBlank(createTimeStart)) param.put("createTimeStart", createTimeStart);
        if(StringUtils.isNotBlank(createTimeEnd)) param.put("createTimeEnd", createTimeEnd);
        if(StringUtils.isNotBlank(channelId)) param.put("channelId", channelId);
        if(StringUtils.isNotBlank(channelType)) param.put("channelType", channelType);
        if(StringUtils.isNotBlank(subMchId)) param.put("subMchId", subMchId);
        if(StringUtils.isNotBlank(phone)) param.put("phone", phone);
        if(StringUtils.isNotBlank(cardPwd)) param.put("cardPwd", cardPwd);
        return payOrderMapper.count4All(param);
    }

    @Override
    public Map count4Success(Long agentId, Long mchId, Long productId, String payOrderId, String mchOrderNo, Byte productType, String createTimeStart, String createTimeEnd,String channelId,String channelType,String subMchId, String phone, String cardPwd) {
        Map param = new HashMap<>();
        if(agentId != null) param.put("agentId", agentId);
        if(mchId != null) param.put("mchId", mchId);
        if(productId != null && productId != -99) param.put("productId", productId);
    ...
    @Override
    public Map count4Fail(Long agentId, Long mchId, Long productId, String payOrderId, String mchOrderNo, Byte productType, String createTimeStart, String createTimeEnd,String channelId,String channelType,String subMchId, String phone, String cardPwd) {
        Map param = new HashMap<>();
        if(agentId != null) param.put("agentId", agentId);
        if(mchId != null) param.put("mchId", mchId);
        if(productId != null && productId != -99) param.put("productId", productId);
    ...
    @Override
    public List<ManagerPayOrderVO> downOrder(Long mchId,PayOrder payOrder, Date createTimeStart, Date createTimeEnd) {
            PayOrderExample example = new PayOrderExample();
            example.setOrderByClause("createTime DESC");
            PayOrderExample.Criteria criteria = example.createCriteria();
            setCriteria(criteria, payOrder, createTimeStart, createTimeEnd);
            if(mchId != null) criteria.andMchIdEqualTo(mchId);
            return payOrderMapper.selectDownLoadOrder(example);
    }
    ...
    void setCriteria(PayOrderExample.Criteria criteria, PayOrder payOrder, Date createTimeStart, Date createTimeEnd) {
        if(payOrder != null) {
            ...
            if(payOrder.getSubMchId() != null) criteria.andSubMchIdEqualTo(payOrder.getSubMchId());
            ...
            if(StringUtils.isNotBlank(payOrder.getPhone())) criteria.andPhoneEqualTo(payOrder.getPhone());
            ...
            if(StringUtils.isNotBlank(payOrder.getChannelId())) criteria.andChannelIdEqualTo(payOrder.getChannelId());
            if(StringUtils.isNotBlank(payOrder.getChannelType())) criteria.andChannelTypeEqualTo(payOrder.getChannelType());
            if(StringUtils.isNotBlank(payOrder.getCardPwd())) criteria.andCardPwdEqualTo(payOrder.getCardPwd());
        }
