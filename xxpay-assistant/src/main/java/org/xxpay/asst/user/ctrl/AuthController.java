...
import org.xxpay.asst.secruity.JwtAuthenticationRequest;
...
@RequestMapping(Constant.ASST_CONTROLLER_ROOT_PATH)
@RestController
public class AuthController extends BaseController {
    ...
    public ResponseEntity<?> authToken(HttpServletRequest request,
        ...
        data.put("AssistantId", AssistantInfo.getAssistantId());
        data.put("loginSecurityType", AssistantInfo.getLoginSecurityType());
    ...
    public ResponseEntity<?> authGoogle(HttpServletRequest request,
        ...
        // 判断商户
        AssistantInfo AssistantInfo = userService.findByAssistantId(AssistantId);
        if(AssistantInfo == null) {
            return ResponseEntity.ok(BizResponse.build(RetEnum.RET_SERVICE_ASST_NOT_EXIST));
        }
        if(MchConstant.PUB_YES != AssistantInfo.getStatus()) {
            return ResponseEntity.ok(BizResponse.build(RetEnum.RET_ASST_STATUS_STOP));
        }
        // 验证谷歌验证码
        boolean checkResult = checkGoogleCode(AssistantInfo.getGoogleAuthSecretKey(), googleCode);
        if(!checkResult) {
    ...
    public ResponseEntity<?> mgrAuthToken(HttpServletRequest request,
        ...
        String rawToken = AssistantId + password + secret;
        ...
        // 生成jwtToken返回
        String jwtToken = jwtTokenUtil.generateToken(AssistantId, String.valueOf(AssistantId));
        JSONObject data = new JSONObject();
